...
class RepoRegistry:
        ...
        async with self._db.session() as s:
            ...
            for cfg in self._default_repos:
                if cfg.name in existing:
                    continue
                s.add(Repo(name=cfg.name, url=cfg.url, branch=cfg.branch, enabled=cfg.enabled))
            await s.commit()
    ...
    async def add_repo(self, name: str, url: str, branch: str = "main", enabled: bool = True) -> Repo:
        async with self._db.session() as s:
            repo = Repo(name=name, url=url, branch=branch, enabled=enabled)
            s.add(repo)
            await s.commit()
